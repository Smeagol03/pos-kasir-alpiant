...
/// Print HTML receipt via Windows Print Spooler (for regular printers like Inkjet/Laser)
#[tauri::command]
pub async fn print_receipt_windows(
    state: tauri::State<'_, AppState>,
    session_token: String,
    ...
    #[cfg(not(target_os = "windows"))]
    {
        // On non-Windows, open in browser for manual print
        #[cfg(unix)]
        {
            let temp_path = std::env::temp_dir().join(format!(
                "pos_receipt_{}.html", 
            ...
            std::fs::write(&temp_path, &html_content)
                .map_err(|e| format!("Gagal buat file: {}", e))?;
...
/// Export receipt to HTML file (can be opened as PDF or printed)
#[tauri::command]
pub async fn export_receipt_pdf(
    app_handle: tauri::AppHandle,
    state: tauri::State<'_, AppState>,
    ...
    let filename = format!(r#"Struk_{}_{}.html"#, 
        transaction_id.split('-').next().unwrap_or("TX"),
        chrono::Local::now().format("%Y%m%d_%H%M%S")
    );
    let file_path = receipts_dir.join(&filename);
...
/// Print HTML content via Windows - opens in browser for manual print
#[cfg(target_os = "windows")]
async fn print_via_windows_html(html_content: &str, printer_name: Option<&str>) -> Result<(), String> {
    use std::io::Write;
    
    // Get default printer if not specified (currently not used - we rely on browser print dialog)
    let _printer = if let Some(name) = printer_name {
        name.to_string()
    } else {
        // Get default printer from Windows
        let ps_output = win_cmd("powershell")
            .args(["-NoProfile", "-Command", 
                "(Get-Printer | Where-Object {$_.Default -eq $true}).Name"])
            .output()
            .map_err(|e| format!("Gagal cek printer default: {}", e))?;
        
        if ps_output.status.success() {
            let printer = String::from_utf8_lossy(&ps_output.stdout).trim().to_string();
            if !printer.is_empty() {
                printer
            } else {
                // Use default - will open browser
                String::new()
            }
        } else {
            String::new()
    ...
    eprintln!("[PRINTER WIN HTML] Printing via browser...");
    ...
    // Wrap with complete HTML for printing
    let full_html = format!(r#"<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    ...
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .header {{ text-align: center; margin-bottom: 15px; padding-bottom: 15px; border-bottom: 2px dashed #000; }}
        .store-name {{ font-size: 18px; font-weight: 900; text-transform: uppercase; }}
        .store-info {{ font-size: 11px; color: #666; margin-top: 5px; }}
        .meta {{ margin-bottom: 15px; padding-bottom: 10px; border-bottom: 1px dashed #ccc; }}
        .meta-row {{ display: flex; justify-content: space-between; margin-bottom: 3px; }}
        .items {{ margin-bottom: 15px; padding-bottom: 10px; border-bottom: 1px dashed #ccc; }}
        .item {{ margin-bottom: 8px; }}
        .item-name {{ font-weight: bold; }}
        .item-details {{ display: flex; justify-content: space-between; font-size: 11px; }}
        .totals {{ margin-bottom: 15px; padding-bottom: 10px; border-bottom: 2px dashed #000; }}
        .total-row {{ display: flex; justify-content: space-between; }}
        .grand-total {{ font-size: 16px; font-weight: 900; border-top: 1px dashed #000; padding-top: 8px; margin-top: 8px; }}
        .payment {{ margin-bottom: 20px; }}
        .footer {{ text-align: center; font-size: 11px; margin-top: 20px; }}
        .thank-you {{ font-weight: bold; text-transform: uppercase; }}
        @media print {{
            body {{ -webkit-print-color-adjust: exact; print-color-adjust: exact; }}
        }}
    </style>
...
<body>
    ...
    <script>
        // Don't auto-print, let user choose printer
    </script>
</body>
</html>"#, html_content);
    
    std::fs::write(&temp_path, full_html)
        .map_err(|e| format!("Gagal buat file temporary: {}", e))?;
    ...
    let _ = win_cmd("cmd")
        .args(["/C", &format!("start /b \"\" \"{}\"", temp_path_str)])
        .output();
    
    eprintln!("[PRINTER WIN] Opened receipt in browser for manual printing");
    Ok(())
}
